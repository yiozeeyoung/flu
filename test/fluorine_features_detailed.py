...
def analyze_fluorine_atoms(data, molecule_name):
    ...
    # 找到氟原子 (原子序数为9)
    atom_numbers = data.x[:, 0].float()  # 第一个特征通常是原子序数
    ...
    for i, f_idx in enumerate(fluorine_indices):
        ...
        fluorine_features = data.x[f_idx].float()
...
def compare_fluorine_vs_other_atoms():
    ...
    for atomic_num, atom_name in atom_types.items():
        ...
        if len(indices) > 0:
            ...
            # 显示第一个该类型原子的特征
            first_atom_features = data.x[indices[0]].float()
...
def feature_statistics_summary():
    ...
    for mol_type, smiles in molecules:
        ...
        if data is not None:
            ...
            fluorine_count = (data.x[:, 0].float() == 9).sum().item()
...
def detailed_feature_analysis():
    ...
    # 使用氟苯作为示例
    data = from_smiles("c1ccc(F)cc1")  # 氟苯
    
    if data is None:
        return
    
    print("以氟苯 (c1ccc(F)cc1) 为例:")
    print(f"原子数: {data.num_nodes}, 键数: {data.num_edges//2}")
    
    # 详细分析每个原子
    atom_numbers = data.x[:, 0].float()
    print(f"\n原子类型分布:")
    for i in range(data.num_nodes):
        atom_type = "氟" if atom_numbers[i] == 9 else "碳"
        print(f"  原子 {i}: {atom_type} (原子序数: {atom_numbers[i].item():.0f})")
    
    # 氟原子的完整特征向量
    fluorine_idx = (atom_numbers == 9).nonzero(as_tuple=True)[0][0]
    print(f"\n氟原子 (索引 {fluorine_idx.item()}) 的完整特征向量:")
    fluorine_features = data.x[fluorine_idx].float()
    ...
    for i, (name, value) in enumerate(zip(feature_names, fluorine_features)):
        print(f"  {name:8}: {value.item():8.3f}")
